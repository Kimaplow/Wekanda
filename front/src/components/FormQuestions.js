    ...
    async function sendRequest(event) {
        ...
        if(add){
            if (fileQuestion !== null) {
                ...
                newId = await axios.post(`http://${config.server}/questions`, data);
            }
            else {
                newId = await axios.post(`http://${config.server}/questions`, {
                    'id_quizz': quizz.id_quizz,
                    'question': ques
                });
            }
            console.log(newId)
            id_ques = newId.data[0].id_question;
            console.log('ID : ', id_ques)
        }
        else {
        ...
        for (let i = 0; i < answers.length; i++) {
            if (tabReponses[i] !== '' || (tabCb[i] !== tabCorrect[i]) || tabFichiersReponses[i] !== null) {
                ...
                if (tabFichiersReponses[i] !== null) {
                    //console.log('PATCH IMAGE : ', tabFichiersReponses[i])
                    data.set('path_file', tabFichiersReponses[i].name);
                    data.append('fileAnswer', tabFichiersReponses[i]);
                }

                console.log('PATCH REPONSE : ', data.get('answer'), data.get('correct'), data.get('path_file'), data.get('fileAnswer'))
        ...
        history.push('/questions/'+id_quizz+'/edit');
    }
